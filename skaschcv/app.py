...
def get_data(endpoint):
    ...
    """
    if not hasattr(flask.g, "json_data"):
        flask.g.json_data = list(
            filter(lambda x: x["endpoint"] == endpoint, flask.read_data())
        )[0]["data"]
    return flask.g.json_data
...
def show_resume():
    ...
    return flask.render_template("cv.html", data=data)
